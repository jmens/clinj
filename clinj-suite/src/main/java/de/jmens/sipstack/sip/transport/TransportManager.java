...
package de.jmens.sipstack.sip.transport;
...
import de.jmens.sipstack.sip.syntaxencoding.SipParser;
...
public class TransportManager {
    ...
    public MessageSender createClientTransport(SipRequest sipRequest,
        ...
        StringBuffer buf = new StringBuffer(RFC3261.DEFAULT_SIP_VERSION);
        buf.append(RFC3261.TRANSPORT_VIA_SEP);
        if (sipRequest.toString().getBytes().length > RFC3261.TRANSPORT_UDP_USUAL_MAX_SIZE) {
            transport = RFC3261.TRANSPORT_TCP;
        }
        buf.append(transport);
        if (inetAddress.isMulticastAddress()) {
            SipHeaderParamName maddrName = new SipHeaderParamName(RFC3261.PARAM_MADDR);
            via.addParam(maddrName, inetAddress.getHostAddress());
            if (inetAddress instanceof Inet4Address) {
                SipHeaderParamName ttlName = new SipHeaderParamName(RFC3261.PARAM_TTL);
                via.addParam(ttlName, RFC3261.IPV4_TTL);
            }
        }
        //RFC3581
        //TODO check config
        ...
        buf.append(RFC3261.TRANSPORT_VIA_SEP2);//space
        ...
        if (sipPort < 1) {
            //use default port
            if (RFC3261.TRANSPORT_TCP.equals(transport) || RFC3261.TRANSPORT_UDP.equals(transport)
                    || RFC3261.TRANSPORT_SCTP.equals(transport)) {
                sipPort = RFC3261.TRANSPORT_DEFAULT_PORT;
            } else if (RFC3261.TRANSPORT_SCTP.equals(transport)) {
                sipPort = RFC3261.TRANSPORT_TLS_PORT;
            } else {
